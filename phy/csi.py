...
from .precoding import build_precoder_matrix
...
def report_csi(
    *,
    ...
    for rank in range(1, rank_limit + 1):
        rank_best = float("-inf")
        for mode in modes:
            precoder = build_precoder_matrix(mode, num_ports=average_channel.shape[1], num_layers=rank)
            ...
            if rank == 1 or capacity > codebook_scores.get(mode, float("-inf")):
                codebook_scores[mode] = capacity
