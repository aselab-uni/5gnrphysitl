...
AVAILABLE_CASE_IDS = ("TC1", "TC2", "TC3", "TC4", "TC5")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--case-id",
        action="append",
        choices=AVAILABLE_CASE_IDS,
        default=[],
        help="Run only the selected testcase(s). Repeat the flag to select multiple cases.",
    )
    parser.add_argument(
        "--list-cases",
        action="store_true",
        help="Print the available testcase IDs and exit.",
    )
    return parser
...
def run_cases(base_config: dict, selected_case_ids: set[str] | None = None) -> pd.DataFrame:
    enabled_case_ids = selected_case_ids or set(AVAILABLE_CASE_IDS)
    rows: List[Dict] = []

    # Case 1: Clean baseline
    if "TC1" in enabled_case_ids:
