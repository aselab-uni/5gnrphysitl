...
| `LAB-12` | Full-option extension planning | Use `PHY Pipeline` plus `docs/FULL_OPTION_PHY_USECASES.md` to map future blocks | No runtime command; this is a design lab | layer mapping, precoding, de-precoding, RF front-end | Students can place each future block in the correct TX/RX position | Which future blocks are still mathematically software-only, and why? |
...
5. End with `LAB-12` to bridge from the current project to the full-option PHY roadmap.
